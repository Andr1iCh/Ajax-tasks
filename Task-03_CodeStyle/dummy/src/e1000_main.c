...
static int e1000_mii_ioctl(struct net_device* netdev,
                           struct ifreq* ifr,
                           int cmd);
...
static void e1000_update_mng_vlan(struct e1000_adapter* adapter) {
  ...
  if (!test_bit(vid, adapter->active_vlans)) {
    if (hw->mng_cookie.status & E1000_MNG_DHCP_COOKIE_STATUS_VLAN_SUPPORT) {
...
void e1000_reset(struct e1000_adapter* adapter) {
  ...
  if (legacy_pba_adjust) {
     ...
     */
    min_tx_space =
        (hw->max_frame_size + sizeof(struct e1000_tx_desc) - ETH_FCS_LEN) * 2;
...
static int e1000_probe(struct pci_dev* pdev, const struct pci_device_id* ent) {
  ...
  if (hw->mac_type >= e1000_82543) {
    netdev->hw_features =
        NETIF_F_SG | NETIF_F_HW_CSUM | NETIF_F_HW_VLAN_CTAG_RX;
  ...
  e_info(probe, "(PCI%s:%dMHz:%d-bit) %pM\n",
         ...
         ((hw->bus_speed == e1000_bus_speed_133)   ? 133
          ...
          : (hw->bus_speed == e1000_bus_speed_66)  ? 66
                                                   : 33),
         ((hw->bus_width == e1000_bus_width_64) ? 64 : 32), netdev->dev_addr);
 ...
 **/
static int e1000_alloc_queues(struct e1000_adapter* adapter) {
  adapter->tx_ring = kzalloc_objs(struct e1000_tx_ring, adapter->num_tx_queues);
  if (!adapter->tx_ring)
    return -ENOMEM;

  adapter->rx_ring = kzalloc_objs(struct e1000_rx_ring, adapter->num_rx_queues);
  if (!adapter->rx_ring) {
    kfree(adapter->tx_ring);
    return -ENOMEM;
  }
...
static int e1000_setup_tx_resources(struct e1000_adapter* adapter,
  ...
  txdr->desc =
      dma_alloc_coherent(&pdev->dev, txdr->size, &txdr->dma, GFP_KERNEL);
  if (!txdr->desc) {
  setup_tx_desc_die:
...
static void e1000_configure_tx(struct e1000_adapter* adapter) {
  ...
  switch (adapter->num_tx_queues) {
    ...
    default:
      ...
      ew32(TDT, 0);
      ew32(TDH, 0);
      adapter->tx_ring[0].tdh =
          ((hw->mac_type >= e1000_82543) ? E1000_TDH : E1000_82542_TDH);
      adapter->tx_ring[0].tdt =
          ((hw->mac_type >= e1000_82543) ? E1000_TDT : E1000_82542_TDT);
      break;
  }
...
static int e1000_setup_rx_resources(struct e1000_adapter* adapter,
  ...
  rxdr->desc =
      dma_alloc_coherent(&pdev->dev, rxdr->size, &rxdr->dma, GFP_KERNEL);
  if (!rxdr->desc) {
  setup_rx_desc_die:
...
static void e1000_configure_rx(struct e1000_adapter* adapter) {
  ...
  switch (adapter->num_rx_queues) {
    ...
    default:
      ...
      ew32(RDT, 0);
      ew32(RDH, 0);
      adapter->rx_ring[0].rdh =
          ((hw->mac_type >= e1000_82543) ? E1000_RDH : E1000_82542_RDH);
      adapter->rx_ring[0].rdt =
          ((hw->mac_type >= e1000_82543) ? E1000_RDT : E1000_82542_RDT);
      break;
  }
...
static void e1000_unmap_and_free_tx_resource(
...
static void e1000_clean_rx_ring(struct e1000_adapter* adapter,
  ...
  for (i = 0; i < rx_ring->count; i++) {
    ...
    if (adapter->clean_rx == e1000_clean_rx_irq) {
      ...
      if (buffer_info->dma)
        dma_unmap_page(&pdev->dev, buffer_info->dma, adapter->rx_buffer_len,
                       DMA_FROM_DEVICE);
...
static void e1000_set_itr(struct e1000_adapter* adapter) {
  ...
  adapter->tx_itr =
      e1000_update_itr(adapter, adapter->tx_itr, adapter->total_tx_packets,
  ...
  adapter->rx_itr =
      e1000_update_itr(adapter, adapter->rx_itr, adapter->total_rx_packets,
...
static int e1000_tx_map(struct e1000_adapter* adapter,
  ...
  while (len) {
     ...
     */
    if (unlikely((hw->bus_type == e1000_bus_type_pcix) && (size > 2015) &&
                 count == 0))
      size = 2015;
...
static netdev_tx_t e1000_xmit_frame(struct sk_buff* skb,
  ...
  for (f = 0; f < nr_frags; f++)
    count +=
        TXD_USE_COUNT(skb_frag_size(&skb_shinfo(skb)->frags[f]), max_txd_pwr);
  if (adapter->pcix_82544)
    count += nr_frags;
...
static void e1000_regdump(struct e1000_adapter* adapter) {
  ...
  static const char* const reg_name[] = {
      "CTRL",   "STATUS",  "RCTL",  "RDLEN",    "RDH",    "RDT",    "RDTR",
      "TCTL",   "TDBAL",   "TDBAH", "TDLEN",    "TDH",    "TDT",    "TIDV",
      "TXDCTL", "TADV",    "TARC0", "TDBAL1",   "TDBAH1", "TDLEN1", "TDH1",
      "TDT1",   "TXDCTL1", "TARC1", "CTRL_EXT", "ERT",    "RDBAL",  "RDBAH",
      "TDFH",   "TDFT",    "TDFHS", "TDFTS",    "TDFPC",  "RDFH",   "RDFT",
      "RDFHS",  "RDFTS",   "RDFPC"};
...
static void e1000_dump(struct e1000_adapter* adapter) {
  ...
  for (i = 0; tx_ring->desc && (i < tx_ring->count); i++) {
    ...
    pr_info("T%c[0x%03X]    %016llX %016llX %016llX %04X  %3X %016llX %p %s\n",
            ((le64_to_cpu(u->b) & (1 << 20)) ? 'd' : 'c'), i, le64_to_cpu(u->a),
            le64_to_cpu(u->b), (u64)buffer_info->dma, buffer_info->length,
...
rx_ring_summary:
   ...
   */
  pr_info(
      "R[desc]      [address 63:0  ] [vl er S cks ln] [bi->dma       ] "
      "[bi->skb]\n");
  ...
  for (i = 0; rx_ring->desc && (i < rx_ring->count); i++) {
    ...
    pr_info("R[0x%03X]     %016llX %016llX %016llX %p %s\n", i,
            le64_to_cpu(u->a), le64_to_cpu(u->b), (u64)buffer_info->dma,
            buffer_info->rxbuf.data, type);
  } /* for */
  ...
  for (i = 0x6000; i <= 0x63FF; i += 0x10) {
    pr_info("R%04X: %08X|%08X %08X|%08X\n", i,
            readl(adapter->hw.hw_addr + i + 4), readl(adapter->hw.hw_addr + i),
...
#define PHY_IDLE_ERROR_COUNT_MASK 0x00FF
  ...
  if (hw->media_type == e1000_media_type_copper) {
    ...
    if ((hw->mac_type <= e1000_82546) && (hw->phy_type == e1000_phy_m88) &&
...
static bool e1000_clean_tx_irq(struct e1000_adapter* adapter,
  ...
  while ((eop_desc->upper.data & cpu_to_le32(E1000_TXD_STAT_DD)) &&
    ...
    for (; !cleaned; count++) {
      ...
      e1000_unmap_and_free_tx_resource(adapter, buffer_info, 64);
  ...
  if (adapter->detect_tx_hung) {
    ...
    if (tx_ring->buffer_info[eop].time_stamp &&
      ...
      e_err(drv,
            ...
            readl(hw->hw_addr + tx_ring->tdt), tx_ring->next_to_use,
 ...
 **/
static void e1000_consume_page(struct e1000_rx_buffer* bi,
                               struct sk_buff* skb,
                               u16 length) {
  bi->rxbuf.page = NULL;
  skb->len += length;
...
static bool e1000_clean_jumbo_rx_irq(struct e1000_adapter* adapter,
  ...
  while (rx_desc->status & E1000_RXD_STAT_DD) {
    ...
    if (unlikely((status & E1000_RXD_STAT_EOP) &&
      ...
      if (e1000_tbi_should_accept(adapter, status, rx_desc->errors, length,
                                  mapped)) {
        length--;
...
#define rxtop rx_ring->rx_skb_top
  process_skb:
    if (!(status & E1000_RXD_STAT_EOP)) {
      ...
      if (rxtop) {
        ...
        if (length <= copybreak) {
          ...
          memcpy(skb_tail_pointer(skb), page_address(p), length);

          /* re-use the page, so don't erase
           * buffer_info->rxbuf.page
           */
          skb_put(skb, length);
          e1000_rx_checksum(adapter, status | rx_desc->errors << 24,
    ...
    e1000_rx_checksum(adapter, (u32)(status) | ((u32)(rx_desc->errors) << 24),
                      le16_to_cpu(rx_desc->csum), skb);
...
static void e1000_alloc_jumbo_rx_buffers(struct e1000_adapter* adapter,
  ...
  while (cleaned_count--) {
    ...
    if (!buffer_info->dma) {
      buffer_info->dma = dma_map_page(&pdev->dev, buffer_info->rxbuf.page, 0,
...
static void e1000_alloc_rx_buffers(struct e1000_adapter* adapter,
  ...
  while (cleaned_count--) {
    ...
    buffer_info->dma = dma_map_single(&pdev->dev, data, adapter->rx_buffer_len,
                                      DMA_FROM_DEVICE);
    if (dma_mapping_error(&pdev->dev, buffer_info->dma)) {
      skb_free_frag(data);
      buffer_info->dma = 0;
    ...
    /* Fix for errata 23, can't cross 64kB boundary */
    if (!e1000_check_64k_bound(adapter, (void*)(unsigned long)buffer_info->dma,
          ...
          case PHY_CTRL:
            ...
            if (mii_reg & MII_CR_AUTO_NEG_EN) {
              ...
              retval = e1000_set_spd_dplx(
                  adapter, speed,
                  ((mii_reg & 0x100) ? DUPLEX_FULL : DUPLEX_HALF));
              if (retval)
                return retval;
            }
...
static bool e1000_vlan_used(struct e1000_adapter* adapter) {
  ...
  for_each_set_bit(vid, adapter->active_vlans, VLAN_N_VID) return true;
  return false;
}
...
#define E1000_CTRL_EN_PHY_PWR_MGMT 0x00200000
      ctrl |= E1000_CTRL_ADVD3WUC | E1000_CTRL_EN_PHY_PWR_MGMT;
      ew32(CTRL, ctrl);
    }
