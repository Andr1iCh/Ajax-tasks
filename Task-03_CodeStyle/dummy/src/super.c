...
static int ext4_load_journal(struct super_block*,
...
static inline void __ext4_read_bh(struct buffer_head* bh,
                                  blk_opf_t op_flags,
                                  bio_end_io_t end_io,
                                  bool simu_fail) {
  if (simu_fail) {
...
void ext4_read_bh_nowait(struct buffer_head* bh,
                         blk_opf_t op_flags,
                         bio_end_io_t end_io,
                         bool simu_fail) {
...
int ext4_read_bh(struct buffer_head* bh,
                 blk_opf_t op_flags,
                 bio_end_io_t end_io,
                 bool simu_fail) {
  BUG_ON(!buffer_locked(bh));
...
struct buffer_head* ext4_sb_bread(struct super_block* sb,
                                  sector_t block,
                                  blk_opf_t op_flags) {
  gfp_t gfp =
      mapping_gfp_constraint(sb->s_bdev->bd_mapping, ~__GFP_FS) | __GFP_MOVABLE;
...
struct buffer_head* ext4_sb_bread_unmovable(struct super_block* sb,
                                            sector_t block) {
  gfp_t gfp = mapping_gfp_constraint(sb->s_bdev->bd_mapping, ~__GFP_FS);
...
void ext4_sb_breadahead_unmovable(struct super_block* sb, sector_t block) {
  struct buffer_head* bh =
      bdev_getblk(sb->s_bdev, block, sb->s_blocksize, GFP_NOWAIT);
...
__u32 ext4_free_inodes_count(struct super_block* sb,
  ...
  return le16_to_cpu(READ_ONCE(bg->bg_free_inodes_count_lo)) |
         (EXT4_DESC_SIZE(sb) >= EXT4_MIN_DESC_SIZE_64BIT
              ? (__u32)le16_to_cpu(READ_ONCE(bg->bg_free_inodes_count_hi)) << 16
              : 0);
}
...
static void ext4_maybe_update_superblock(struct super_block* sb) {
  ...
  if (ext4_emergency_state(sb) || sb_rdonly(sb) || !(sb->s_flags & SB_ACTIVE) ||
...
static bool ext4_journalled_writepage_needs_redirty(struct jbd2_inode* jinode,
  ...
  do {
    ...
    jh = bh2jh(bh);
    if (buffer_dirty(bh) ||
        (jh && (jh->b_transaction != trans || jh->b_next_transaction)))
      return true;
...
static int ext4_journalled_submit_inode_data_buffers(
...
static struct ext4_err_translation err_translation[] = {
    ...
    EXT4_ERR_TRANSLATE(EFSBADCRC), EXT4_ERR_TRANSLATE(EFSCORRUPTED),
    EXT4_ERR_TRANSLATE(ENOSPC),    EXT4_ERR_TRANSLATE(ENOKEY),
    EXT4_ERR_TRANSLATE(EROFS),     EXT4_ERR_TRANSLATE(EFBIG),
    EXT4_ERR_TRANSLATE(EEXIST),    EXT4_ERR_TRANSLATE(ERANGE),
    EXT4_ERR_TRANSLATE(EOVERFLOW), EXT4_ERR_TRANSLATE(EBUSY),
    EXT4_ERR_TRANSLATE(ENOTDIR),   EXT4_ERR_TRANSLATE(ENOTEMPTY),
    EXT4_ERR_TRANSLATE(ESHUTDOWN), EXT4_ERR_TRANSLATE(EFAULT),
};
...
static void save_error_info(struct super_block* sb,
                            int error,
                            __u32 ino,
                            __u64 block,
                            const char* func,
...
static void update_super_work(struct work_struct* work) {
  struct ext4_sb_info* sbi =
      container_of(work, struct ext4_sb_info, s_sb_upd_work);
  journal_t* journal = sbi->s_journal;
  handle_t* handle;
   ...
   */
  if (!ext4_emergency_state(sbi->s_sb) && !sb_rdonly(sbi->s_sb) && journal) {
...
void __ext4_error(struct super_block* sb,
  ...
  fserror_report_metadata(sb, error ? -abs(error) : -EFSCORRUPTED, GFP_ATOMIC);

  ext4_handle_error(sb, force_ro, error, 0, block, function, line);
}

void __ext4_error_inode(struct inode* inode,
                        const char* function,
                        unsigned int line,
                        ext4_fsblk_t block,
                        int error,
  ...
  if (ext4_error_ratelimit(inode->i_sb)) {
    ...
    if (block)
      printk(KERN_CRIT
             ...
             inode->i_sb->s_id, function, line, inode->i_ino, block,
             current->comm, &vaf);
    else
      printk(KERN_CRIT
             "EXT4-fs error (device %s): %s:%d: "
  ...
  fserror_report_file_metadata(inode, error ? -abs(error) : -EFSCORRUPTED,
                               GFP_ATOMIC);

  ext4_handle_error(inode->i_sb, false, error, inode->i_ino, block, function,
                    line);
}

void __ext4_error_file(struct file* file,
                       const char* function,
                       unsigned int line,
                       ext4_fsblk_t block,
                       const char* fmt,
...
const char* ext4_decode_error(struct super_block* sb,
                              int errno,
                              char nbuf[16]) {
  char* errstr = NULL;
...
void __ext4_std_error(struct super_block* sb,
  ...
  fserror_report_metadata(sb, errno ? -abs(errno) : -EFSCORRUPTED, GFP_ATOMIC);
...
void __ext4_warning_inode(const struct inode* inode,
                          const char* function,
                          unsigned int line,
                          const char* fmt,
                          ...) {
...
void __ext4_grp_locked_error(const char* function,
                             ...
                             ext4_fsblk_t block,
                             const char* fmt,
                             ...) __releases(bitlock) __acquires(bitlock) {
  struct va_format vaf;
  va_list args;
  ...
  if (ext4_error_ratelimit(sb)) {
    ...
    vaf.fmt = fmt;
    vaf.va = &args;
    printk(KERN_CRIT "EXT4-fs error (device %s): %s:%d: group %u, ", sb->s_id,
           function, line, grp);
    if (ino)
...
static void dump_orphan_list(struct super_block* sb, struct ext4_sb_info* sbi) {
  ...
  list_for_each(l, &sbi->s_orphan) {
    ...
    printk(KERN_ERR
           ...
           inode->i_sb->s_id, inode->i_ino, inode, inode->i_mode,
           inode->i_nlink, NEXT_ORPHAN(inode));
  }
}
...
static int ext4_percpu_param_init(struct ext4_sb_info* sbi) {
  ...
  if (!err)
    err = percpu_counter_init(&sbi->s_dirs_counter, ext4_count_dirs(sbi->s_sb),
                              GFP_KERNEL);
  if (!err)
    err = percpu_counter_init(&sbi->s_dirtyclusters_counter, 0, GFP_KERNEL);
  if (!err)
    err = percpu_counter_init(&sbi->s_sra_exceeded_retry_limit, 0, GFP_KERNEL);
  if (!err)
...
static void ext4_free_in_core_inode(struct inode* inode) {
  ...
  if (!list_empty(&(EXT4_I(inode)->i_fc_list))) {
    pr_warn("%s: inode %llu still in fc list", __func__, inode->i_ino);
  }
...
static void ext4_destroy_inode(struct inode* inode) {
  if (ext4_inode_orphan_tracked(inode)) {
    ext4_msg(inode->i_sb, KERN_ERR, "Inode %llu (%p): inode tracked as orphan!",
...
static int __init init_inodecache(void) {
  ...
  ext4_inode_cachep =
      kmem_cache_create("ext4_inode_cache", sizeof(struct ext4_inode_info),
                        &args, SLAB_RECLAIM_ACCOUNT | SLAB_ACCOUNT);
...
static struct dentry* ext4_fh_to_dentry(struct super_block* sb,
                                        struct fid* fid,
                                        int fh_len,
                                        int fh_type) {
...
static struct dentry* ext4_fh_to_parent(struct super_block* sb,
                                        struct fid* fid,
                                        int fh_len,
                                        int fh_type) {
...
static int ext4_quota_on(struct super_block* sb,
                         int type,
                         int format_id,
                         const struct path* path);
static ssize_t ext4_quota_read(struct super_block* sb,
                               int type,
                               char* data,
                               size_t len,
                               loff_t off);
static ssize_t ext4_quota_write(struct super_block* sb,
                                int type,
                                const char* data,
                                size_t len,
                                loff_t off);
static int ext4_quota_enable(struct super_block* sb,
                             int type,
                             int format_id,
                             unsigned int flags);
...
static const struct fs_parameter_spec ext4_param_specs[] = {
    ...
    fsparam_enum("data_err", Opt_data_err, ext4_param_data_err),
    ...
    fsparam_flag("no_prefetch_block_bitmaps", Opt_no_prefetch_block_bitmaps),
...
static const struct mount_opts {
  ...
  int flags;
    ...
    {Opt_journal_checksum, EXT4_MOUNT_JOURNAL_CHECKSUM,
     MOPT_EXT4_ONLY | MOPT_SET | MOPT_EXPLICIT},
    {Opt_journal_async_commit,
     (EXT4_MOUNT_JOURNAL_ASYNC_COMMIT | EXT4_MOUNT_JOURNAL_CHECKSUM),
     MOPT_EXT4_ONLY | MOPT_SET | MOPT_EXPLICIT},
    ...
    {Opt_noquota,
     (EXT4_MOUNT_QUOTA | EXT4_MOUNT_USRQUOTA | EXT4_MOUNT_GRPQUOTA |
      EXT4_MOUNT_PRJQUOTA),
     MOPT_CLEAR | MOPT_Q},
...
static const struct ext4_sb_encodings* ext4_sb_read_encoding(
    const struct ext4_super_block* es) {
 ...
 */
static int note_qf_name(struct fs_context* fc,
                        int qtype,
  ...
  if (strchr(param->string, '/')) {
    ext4_msg(NULL, KERN_ERR, "quotafile must be on filesystem root");
    return -EINVAL;
  }
  ...
  if (!qname) {
    ext4_msg(NULL, KERN_ERR, "Not enough memory for storing quotafile name");
    return -ENOMEM;
  }
...
static int ext4_parse_test_dummy_encryption(const struct fs_parameter* param,
  ...
  if (!IS_ENABLED(CONFIG_FS_ENCRYPTION)) {
    ext4_msg(NULL, KERN_WARNING, "test_dummy_encryption option not supported");
    return -EINVAL;
  }
  err = fscrypt_parse_test_dummy_encryption(param, &ctx->dummy_enc_policy);
  if (err == -EINVAL) {
    ext4_msg(NULL, KERN_WARNING, "Value of option \"%s\" is unrecognized",
             param->key);
  } else if (err == -EEXIST) {
    ext4_msg(NULL, KERN_WARNING, "Conflicting test_dummy_encryption options");
    return -EINVAL;
  }
  return err;
}
...
#define EXT4_SET_CTX(name)                               \
  static inline __maybe_unused void ctx_set_##name(      \
      struct ext4_fs_context* ctx, unsigned long flag) { \
    ctx->mask_s_##name |= flag;                          \
    ctx->vals_s_##name |= flag;                          \
  }

#define EXT4_CLEAR_CTX(name)                             \
  static inline __maybe_unused void ctx_clear_##name(    \
      struct ext4_fs_context* ctx, unsigned long flag) { \
    ctx->mask_s_##name |= flag;                          \
    ctx->vals_s_##name &= ~flag;                         \
  }
...
#define EXT4_TEST_CTX(name)                                                \
  static inline unsigned long ctx_test_##name(struct ext4_fs_context* ctx, \
                                              unsigned long flag) {        \
...
static int ext4_parse_param(struct fs_context* fc, struct fs_parameter* param) {
  ...
  if (m->flags & MOPT_EXPLICIT) {
    if (m->mount_opt & EXT4_MOUNT_DELALLOC) {
      ...
      ctx_set_mount_opt2(ctx, EXT4_MOUNT2_EXPLICIT_JOURNAL_CHECKSUM);
    } else
      return -EINVAL;
  }

  if (m->flags & MOPT_NOSUPPORT) {
    ext4_msg(NULL, KERN_ERR, "%s option not supported", param->key);
    return 0;
  }
  ...
  switch (token) {
    ...
    case Opt_sb:
      if (fc->purpose == FS_CONTEXT_FOR_RECONFIGURE) {
        ext4_msg(NULL, KERN_WARNING, "Ignoring %s option on remount",
                 param->key);
      } else {
      ...
      return 0;
    case Opt_removed:
      ext4_msg(NULL, KERN_WARNING, "Ignoring removed %s option", param->key);
      return 0;
    ...
    case Opt_debug_want_extra_isize:
      if ((result.uint_32 & 1) || (result.uint_32 < 4)) {
        ext4_msg(NULL, KERN_ERR, "Invalid want_extra_isize %d", result.uint_32);
        return -EINVAL;
      }
    ...
    case Opt_inode_readahead_blks:
      if (result.uint_32 &&
          (result.uint_32 > (1 << 30) || !is_power_of_2(result.uint_32))) {
        ext4_msg(NULL, KERN_ERR,
    ...
    case Opt_journal_dev:
      if (is_remount) {
        ext4_msg(NULL, KERN_ERR, "Cannot specify journal on remount");
        return -EINVAL;
      }
    ...
    case Opt_journal_ioprio:
      ...
      ctx->journal_ioprio = IOPRIO_PRIO_VALUE(IOPRIO_CLASS_BE, result.uint_32);
...
#ifdef CONFIG_FS_DAX
    {
      int type = (token == Opt_dax) ? Opt_dax : result.uint_32;
   ...
   */
  if (m->token == Opt_err) {
    ext4_msg(NULL, KERN_WARNING, "buggy handling of option %s", param->key);
    WARN_ON(1);
    return -EINVAL;
  }
    ...
    if ((param->type == fs_value_is_flag) || result.uint_32 > 0)
      set = 1;
...
static int parse_apply_sb_mount_options(struct super_block* sb,
  ...
  if (ret < 0) {
  parse_failed:
    ext4_msg(sb, KERN_WARNING, "failed to parse options in superblock: %s",
             s_mount_opts);
    ret = 0;
    goto out_free;
  }
...
static int ext4_check_quota_consistency(struct fs_context* fc,
  ...
  quota_flags = EXT4_MOUNT_QUOTA | EXT4_MOUNT_USRQUOTA | EXT4_MOUNT_GRPQUOTA |
                EXT4_MOUNT_PRJQUOTA;
  if (quota_loaded && ctx->mask_s_mount_opt & quota_flags &&
  ...
  if (ctx->spec & EXT4_SPEC_JQUOTA) {
    for (i = 0; i < EXT4_MAXQUOTAS; i++) {
      ...
      if (quota_loaded && !!sbi->s_qf_names[i] != !!ctx->s_qf_names[i])
        goto err_jquota_change;

      if (sbi->s_qf_names[i] && ctx->s_qf_names[i] &&
          strcmp(get_qf_name(sb, sbi, i), ctx->s_qf_names[i]) != 0)
  ...
  usr_qf_name = (get_qf_name(sb, sbi, USRQUOTA) || ctx->s_qf_names[USRQUOTA]);
  grp_qf_name = (get_qf_name(sb, sbi, GRPQUOTA) || ctx->s_qf_names[GRPQUOTA]);

  usrquota =
      (ctx_test_mount_opt(ctx, EXT4_MOUNT_USRQUOTA) || test_opt(sb, USRQUOTA));

  grpquota =
      (ctx_test_mount_opt(ctx, EXT4_MOUNT_GRPQUOTA) || test_opt(sb, GRPQUOTA));
...
err_quota_change:
  ext4_msg(NULL, KERN_ERR, "Cannot change quota options when quota turned on");
  return -EINVAL;
err_jquota_change:
  ext4_msg(NULL, KERN_ERR,
...
err_jquota_specified:
  ext4_msg(NULL, KERN_ERR, "%s quota file already specified", QTYPE2NAME(i));
  return -EINVAL;
#else
  return 0;
#endif
...
static int ext4_check_opt_consistency(struct fs_context* fc,
  ...
  if ((ctx->opt_flags & MOPT_NO_EXT2) && IS_EXT2_SB(sb)) {
    ext4_msg(NULL, KERN_ERR, "Mount option(s) incompatible with ext2");
    return -EINVAL;
  }
  if ((ctx->opt_flags & MOPT_NO_EXT3) && IS_EXT3_SB(sb)) {
    ext4_msg(NULL, KERN_ERR, "Mount option(s) incompatible with ext3");
    return -EINVAL;
  }

  if (ctx->s_want_extra_isize >
      (sbi->s_inode_size - EXT4_GOOD_OLD_INODE_SIZE)) {
    ext4_msg(NULL, KERN_ERR, "Invalid want_extra_isize %d",
  ...
  if (is_remount) {
    if (!sbi->s_journal && ctx_test_mount_opt(ctx, EXT4_MOUNT_DATA_ERR_ABORT)) {
      ext4_msg(NULL, KERN_WARNING,
 ...
 */
static int _ext4_show_options(struct seq_file* seq,
                              struct super_block* sb,
                              int nodefs) {
  struct ext4_sb_info* sbi = EXT4_SB(sb);
  struct ext4_super_block* es = sbi->s_es;
...
#define SEQ_OPTS_PRINT(str, arg) seq_printf(seq, "%c" str, sep, arg)
  ...
  for (m = ext4_mount_opts; m->token != Opt_err; m++) {
    ...
    if ((want_set && (mount_opt & m->mount_opt) != m->mount_opt) ||
  ...
  if (nodefs ||
      !uid_eq(sbi->s_resuid, make_kuid(&init_user_ns, EXT4_DEF_RESUID)) ||
      ext4_get_resuid(es) != EXT4_DEF_RESUID)
    SEQ_OPTS_PRINT("resuid=%u", from_kuid_munged(&init_user_ns, sbi->s_resuid));
  if (nodefs ||
      !gid_eq(sbi->s_resgid, make_kgid(&init_user_ns, EXT4_DEF_RESGID)) ||
  ...
  if (nodefs ||
      EXT4_MOUNT_DATA_FLAGS & (sbi->s_mount_opt ^ sbi->s_def_mount_opt)) {
...
static int ext4_setup_super(struct super_block* sb,
                            struct ext4_super_block* es,
                            int read_only) {
  struct ext4_sb_info* sbi = EXT4_SB(sb);
  int err = 0;
...
int ext4_alloc_flex_bg_array(struct super_block* sb, ext4_group_t ngroup) {
  ...
  new_groups = kvzalloc(roundup_pow_of_two(size * sizeof(*sbi->s_flex_groups)),
                        GFP_KERNEL);
  if (!new_groups) {
    ext4_msg(sb, KERN_ERR, "not enough memory for %d flex group pointers",
             size);
    return -ENOMEM;
  }
  for (i = sbi->s_flex_groups_allocated; i < size; i++) {
    new_groups[i] =
        kvzalloc(roundup_pow_of_two(sizeof(struct flex_groups)), GFP_KERNEL);
    if (!new_groups[i]) {
      for (j = sbi->s_flex_groups_allocated; j < i; j++)
        kvfree(new_groups[j]);
      kvfree(new_groups);
      ext4_msg(sb, KERN_ERR, "not enough memory for %d flex groups", size);
      return -ENOMEM;
    }
  }
  rcu_read_lock();
...
static int ext4_fill_flex_info(struct super_block* sb) {
  ...
  for (i = 0; i < sbi->s_groups_count; i++) {
    ...
    atomic64_add(ext4_free_group_clusters(sb, gdp), &fg->free_clusters);
...
static __le16 ext4_group_desc_csum(struct super_block* sb,
                                   __u32 block_group,
  ...
  if (ext4_has_feature_metadata_csum(sbi->s_sb)) {
    ...
    csum32 = ext4_chksum(sbi->s_csum_seed, (__u8*)&le_group, sizeof(le_group));
    csum32 = ext4_chksum(csum32, (__u8*)gdp, offset);
    csum32 = ext4_chksum(csum32, (__u8*)&dummy_csum, sizeof(dummy_csum));
    offset += sizeof(dummy_csum);
    if (offset < sbi->s_desc_size)
      csum32 =
          ext4_chksum(csum32, (__u8*)gdp + offset, sbi->s_desc_size - offset);
  ...
  if (ext4_has_feature_64bit(sb) && offset < sbi->s_desc_size)
    crc = crc16(crc, (__u8*)gdp + offset, sbi->s_desc_size - offset);
...
int ext4_feature_set_ok(struct super_block* sb, int readonly) {
  ...
  if (!readonly &&
      (ext4_has_feature_quota(sb) || ext4_has_feature_project(sb))) {
    ext4_msg(sb, KERN_ERR,
...
void print_daily_error_info(struct timer_list* t) {
  ...
  if (es->s_first_error_time) {
    printk(KERN_NOTICE "EXT4-fs (%s): initial error at time %llu: %.*s:%d",
           sb->s_id, ext4_get_tstamp(es, s_first_error_time),
           (int)sizeof(es->s_first_error_func), es->s_first_error_func,
  ...
  if (es->s_last_error_time) {
    printk(KERN_NOTICE "EXT4-fs (%s): last error at time %llu: %.*s:%d",
           sb->s_id, ext4_get_tstamp(es, s_last_error_time),
           (int)sizeof(es->s_last_error_func), es->s_last_error_func,
...
static int ext4_run_li_request(struct ext4_li_request* elr) {
  ...
  if (elr->lr_mode == EXT4_LI_MODE_PREFETCH_BBITMAP) {
    ...
    if (group >= elr->lr_next_group) {
      ret = 1;
      if (elr->lr_first_not_zeroed != ngroups && !ext4_emergency_state(sb) &&
...
cont_thread:
  while (true) {
    ...
    list_for_each_safe(pos, n, &eli->li_request_list) {
      ...
      if (!progress) {
        elr->lr_next_sched =
            jiffies + get_random_u32_below(EXT4_DEF_LI_MAX_START_DELAY * HZ);
      }
      if (!next_wakeup_initialized ||
          time_before(elr->lr_next_sched, next_wakeup)) {
        next_wakeup = elr->lr_next_sched;
...
static int ext4_run_lazyinit_thread(void) {
  ext4_lazyinit_task =
      kthread_run(ext4_lazyinit_thread, ext4_li_info, "ext4lazyinit");
...
static int set_journal_csum_feature_set(struct super_block* sb) {
  ...
  jbd2_journal_clear_features(
      sbi->s_journal, JBD2_FEATURE_COMPAT_CHECKSUM, 0,
      JBD2_FEATURE_INCOMPAT_CSUM_V3 | JBD2_FEATURE_INCOMPAT_CSUM_V2);
  if (test_opt(sb, JOURNAL_ASYNC_COMMIT)) {
    ret = jbd2_journal_set_features(
        sbi->s_journal, compat, 0,
...
static void ext4_setup_csum_trigger(
    struct super_block* sb,
    enum ext4_journal_trigger_type type,
    void (*trigger)(struct jbd2_buffer_trigger_type* type,
                    struct buffer_head* bh,
                    void* mapped_data,
                    size_t size)) {
  struct ext4_sb_info* sbi = EXT4_SB(sb);
...
static struct ext4_sb_info* ext4_alloc_sbi(struct super_block* sb) {
  ...
  sbi->s_daxdev =
      fs_dax_get_by_bdev(sb->s_bdev, &sbi->s_dax_part_off, NULL, NULL);

  sbi->s_blockgroup_lock = kzalloc_obj(struct blockgroup_lock);
...
static void ext4_atomic_write_init(struct super_block* sb) {
  ...
  sbi->s_awu_min = max(sb->s_blocksize, bdev_atomic_write_unit_min_bytes(bdev));
  sbi->s_awu_max = min(clustersize, bdev_atomic_write_unit_max_bytes(bdev));
  if (sbi->s_awu_min && sbi->s_awu_max && sbi->s_awu_min <= sbi->s_awu_max) {
    ext4_msg(
        sb, KERN_NOTICE,
...
static int ext4_inode_info_init(struct super_block* sb,
  ...
  if (le32_to_cpu(es->s_rev_level) == EXT4_GOOD_OLD_REV) {
     ...
     */
    if (sbi->s_inode_size >=
        offsetof(struct ext4_inode, i_atime_extra) +
  ...
  if (sbi->s_inode_size > EXT4_GOOD_OLD_INODE_SIZE) {
    sbi->s_want_extra_isize =
        sizeof(struct ext4_inode) - EXT4_GOOD_OLD_INODE_SIZE;
    if (ext4_has_feature_extra_isize(sb)) {
      unsigned v, max = (sbi->s_inode_size - EXT4_GOOD_OLD_INODE_SIZE);

      v = le16_to_cpu(es->s_want_extra_isize);
      if (v > max) {
        ext4_msg(sb, KERN_ERR, "bad s_want_extra_isize: %d", v);
        return -EINVAL;
      }
      ...
      if (v > max) {
        ext4_msg(sb, KERN_ERR, "bad s_min_extra_isize: %d", v);
        return -EINVAL;
      }
      if (sbi->s_want_extra_isize < v)
        sbi->s_want_extra_isize = v;
...
#if IS_ENABLED(CONFIG_UNICODE)
static int ext4_encoding_init(struct super_block* sb,
                              struct ext4_super_block* es) {
  ...
  if (!encoding_info) {
    ext4_msg(sb, KERN_ERR, "Encoding requested by superblock is unknown");
    return -EINVAL;
  }
...
#else
static inline int ext4_encoding_init(struct super_block* sb,
                                     struct ext4_super_block* es) {
  return 0;
}
#endif

static int ext4_init_metadata_csum(struct super_block* sb,
                                   struct ext4_super_block* es) {
  struct ext4_sb_info* sbi = EXT4_SB(sb);
  ...
  else if (ext4_has_feature_metadata_csum(sb) || ext4_has_feature_ea_inode(sb))
    sbi->s_csum_seed = ext4_chksum(~0, es->s_uuid, sizeof(es->s_uuid));
  return 0;
}
...
static int ext4_check_geometry(struct super_block* sb,
  ...
  if (blocks_count > ((uint64_t)1 << 32) - EXT4_DESC_PER_BLOCK(sb)) {
    ext4_msg(sb, KERN_WARNING,
             ...
             le32_to_cpu(es->s_first_data_block), EXT4_BLOCKS_PER_GROUP(sb));
    return -EINVAL;
  }
  sbi->s_groups_count = blocks_count;
  sbi->s_blockfile_groups =
...
static int ext4_load_and_init_journal(struct super_block* sb,
  ...
  if (test_opt2(sb, JOURNAL_FAST_COMMIT) &&
    ...
    ext4_msg(sb, KERN_ERR, "Failed to set fast commit journal feature");
    goto out;
  }
...
static int ext4_load_super(struct super_block* sb,
                           ext4_fsblk_t* lsb,
                           int silent) {
  struct ext4_sb_info* sbi = EXT4_SB(sb);
  struct ext4_super_block* es;
  ...
  if (le32_to_cpu(es->s_log_block_size) >
    ...
    ext4_msg(sb, KERN_ERR, "Invalid log block size: %u",
             le32_to_cpu(es->s_log_block_size));
    goto out;
  }
  if (le32_to_cpu(es->s_log_cluster_size) >
      (EXT4_MAX_CLUSTER_LOG_SIZE - EXT4_MIN_BLOCK_LOG_SIZE)) {
    ext4_msg(sb, KERN_ERR, "Invalid log cluster size: %u",
...
static int ext4_hash_info_init(struct super_block* sb) {
  ...
  if (sbi->s_def_hash_version > DX_HASH_LAST) {
    ext4_msg(sb, KERN_ERR, "Invalid default hash set in the superblock");
    return -EINVAL;
  } else if (sbi->s_def_hash_version == DX_HASH_SIPHASH) {
    ext4_msg(sb, KERN_ERR, "SIPHASH is not a valid default hash value");
    return -EINVAL;
  }
...
static int ext4_block_group_meta_init(struct super_block* sb, int silent) {
  ...
  sbi->s_bitmap_maxbytes =
      ext4_max_bitmap_size(sb->s_blocksize_bits, has_huge_files);
 ...
 */
static bool ext4_is_stripe_incompatible(struct super_block* sb,
...
static int __ext4_fill_super(struct fs_context* fc, struct super_block* sb) {
  ...
  needs_recovery =
      (es->s_last_orphan != 0 || ext4_has_feature_orphan_present(sb) ||
  ...
  if (test_opt(sb, DISCARD) && !bdev_max_discard_sectors(sb->s_bdev)) {
    ext4_msg(sb, KERN_WARNING,
             "mounting with \"discard\" option, but the device does not "
             "support discard");
...
failed_mount9:
  ...
  ext4_quotas_off(sb, EXT4_MAXQUOTAS);
failed_mount8:
  __maybe_unused ext4_release_orphan_info(sb);
failed_mount7:
  ext4_unregister_li_request(sb);
failed_mount6:
  ext4_mb_release(sb);
...
static int ext4_fill_super(struct super_block* sb, struct fs_context* fc) {
  ...
  if (___ratelimit(&ext4_mount_msg_ratelimit, "EXT4-fs mount"))
    ext4_msg(sb, KERN_INFO,
             ...
             "Quota mode: %s.",
             &sb->s_uuid, sb_rdonly(sb) ? "ro" : "r/w", descr,
             ext4_quota_mode(sb));
 ...
 */
static void ext4_init_journal_params(struct super_block* sb,
                                     journal_t* journal) {
  struct ext4_sb_info* sbi = EXT4_SB(sb);
...
#ifdef CONFIG_LOCKDEP
  lockdep_set_subclass(&EXT4_I(journal_inode)->i_data_sem, I_DATA_SEM_JOURNAL);
#endif
...
static struct file* ext4_get_journal_blkdev(struct super_block* sb,
  ...
  bdev_file = fs_bdev_file_open_by_dev(
      j_dev, BLK_OPEN_READ | BLK_OPEN_WRITE | BLK_OPEN_RESTRICT_WRITES, sb, sb);
  if (IS_ERR(bdev_file)) {
    ext4_msg(sb, KERN_ERR,
...
static journal_t* ext4_open_dev_journal(struct super_block* sb, dev_t j_dev) {
  journal_t* journal;
  ext4_fsblk_t j_start;
  ext4_fsblk_t j_len;
  struct file* bdev_file;
...
static int ext4_load_journal(struct super_block* sb,
  ...
  if (!err) {
    ...
    if (save && memcmp(((char*)es) + EXT4_S_ERR_START, save, EXT4_S_ERR_LEN)) {
...
static void ext4_update_super(struct super_block* sb) {
  ...
  if (percpu_counter_initialized(&sbi->s_freeclusters_counter))
    ext4_free_blocks_count_set(
        es, EXT4_C2B(sbi, percpu_counter_sum_positive(
                              &sbi->s_freeclusters_counter)));
  if (percpu_counter_initialized(&sbi->s_freeinodes_counter))
    es->s_free_inodes_count =
        cpu_to_le32(percpu_counter_sum_positive(&sbi->s_freeinodes_counter));
  /* Copy error information to the on-disk superblock */
  spin_lock(&sbi->s_error_lock);
  if (sbi->s_add_error_count > 0) {
    es->s_state |= cpu_to_le16(EXT4_ERROR_FS);
    if (!es->s_first_error_time && !es->s_first_error_time_hi) {
      __ext4_update_tstamp(&es->s_first_error_time, &es->s_first_error_time_hi,
                           sbi->s_first_error_time);
      strtomem_pad(es->s_first_error_func, sbi->s_first_error_func, 0);
      ...
      es->s_first_error_errcode = ext4_errno_to_code(sbi->s_first_error_code);
    }
    __ext4_update_tstamp(&es->s_last_error_time, &es->s_last_error_time_hi,
                         sbi->s_last_error_time);
    strtomem_pad(es->s_last_error_func, sbi->s_last_error_func, 0);
    es->s_last_error_line = cpu_to_le32(sbi->s_last_error_line);
    es->s_last_error_ino = cpu_to_le32(sbi->s_last_error_ino);
    es->s_last_error_block = cpu_to_le64(sbi->s_last_error_block);
    es->s_last_error_errcode = ext4_errno_to_code(sbi->s_last_error_code);
    /*
...
static int ext4_commit_super(struct super_block* sb) {
  ...
  bh_submit(sbh,
            REQ_OP_WRITE | REQ_SYNC | (test_opt(sb, BARRIER) ? REQ_FUA : 0),
            bh_end_write);
  wait_on_buffer(sbh);
...
static int __ext4_remount(struct fs_context* fc, struct super_block* sb) {
  ...
  if (!(ctx->spec & EXT4_SPEC_JOURNAL_IOPRIO)) {
    if (sbi->s_journal && sbi->s_journal->j_task->io_context)
      ctx->journal_ioprio = sbi->s_journal->j_task->io_context->ioprio;
    else
      ctx->journal_ioprio = EXT4_DEF_JOURNAL_IOPRIO;
  }
    ...
    if (fc->sb_flags & SB_RDONLY) {
      ...
      for (g = 0; g < sbi->s_groups_count; g++) {
        ...
        if (!ext4_group_desc_csum_verify(sb, g, gdp)) {
          ext4_msg(sb, KERN_ERR,
                   "ext4_remount: Checksum for group %u failed (%u!=%u)", g,
...
static int ext4_reconfigure(struct fs_context* fc) {
  ...
  ext4_msg(sb, KERN_INFO, "re-mounted %pU%s.", &sb->s_uuid,
...
static int ext4_statfs_project(struct super_block* sb,
  ...
  limit =
      min_not_zero(dquot->dq_dqb.dqb_bsoftlimit, dquot->dq_dqb.dqb_bhardlimit);
  limit >>= sb->s_blocksize_bits;

  if (limit) {
    uint64_t remaining = 0;

    curblock = (dquot->dq_dqb.dqb_curspace + dquot->dq_dqb.dqb_rsvspace) >>
               sb->s_blocksize_bits;
    if (limit > curblock)
  ...
  limit =
      min_not_zero(dquot->dq_dqb.dqb_isoftlimit, dquot->dq_dqb.dqb_ihardlimit);
  if (limit) {
    uint64_t remaining = 0;
...
static int ext4_write_dquot(struct dquot* dquot) {
  ...
  if (ret < 0)
    ext4_error_err(dquot->dq_sb, -ret, "Failed to commit dquot type %d",
                   dquot->dq_id.type);
...
static int ext4_acquire_dquot(struct dquot* dquot) {
  ...
  if (ret < 0)
    ext4_error_err(dquot->dq_sb, -ret, "Failed to acquire dquot type %d",
                   dquot->dq_id.type);
  err = ext4_journal_stop(handle);
  if (!ret)
    ret = err;
...
static int ext4_quota_enable(struct super_block* sb,
  ...
  if (!ext4_check_quota_inum(type, qf_inums[type])) {
    ext4_error(sb, "Bad quota inum: %lu, type: %d", qf_inums[type], type);
    return -EUCLEAN;
  }

  qf_inode = ext4_iget(sb, qf_inums[type], EXT4_IGET_SPECIAL);
  if (IS_ERR(qf_inode)) {
    ext4_error(sb, "Bad quota inode: %lu, type: %d", qf_inums[type], type);
    ...
    if (qf_inums[type]) {
      ...
      if (err) {
        ext4_warning(sb,
                     ...
                     "(type=%d, err=%d, ino=%lu). "
                     "Please run e2fsck to fix.",
                     type, err, qf_inums[type]);
...
MODULE_AUTHOR(
    "Remy Card, Stephen Tweedie, Andrew Morton, Andreas Dilger, Theodore Ts'o "
    "and others");
