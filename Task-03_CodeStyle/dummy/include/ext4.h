...
#define ext4_debug(f, a...)                                                \
	do {                                                               \
		printk(KERN_DEBUG "EXT4-fs DEBUG (%s, %d): %s:", __FILE__, \
		       __LINE__, __func__);                                \
...
#define ext_debug(ino, fmt, ...)                                       \
	pr_debug("[%s/%d] EXT4-fs (%s): ino %llu: (%s, %d): %s:" fmt,  \
		 current->comm, task_pid_nr(current), ino->i_sb->s_id, \
		 ino->i_ino, __FILE__, __LINE__, __func__, ##__VA_ARGS__)
#else
...
#define EXT4_MAP_FLAGS                                         \
	(EXT4_MAP_NEW | EXT4_MAP_MAPPED | EXT4_MAP_UNWRITTEN | \
	 EXT4_MAP_BOUNDARY | EXT4_MAP_DELAYED | EXT4_MAP_QUERY_LAST_IN_LEAF)
...
#define EXT4_MAX_BLOCKS(size, offset, blkbits)                   \
	((EXT4_BLOCK_ALIGN(size + offset, blkbits) >> blkbits) - \
	 (offset >> blkbits))
...
#define EXT4_FL_USER_MODIFIABLE                                        \
	(EXT4_SECRM_FL | EXT4_UNRM_FL | EXT4_COMPR_FL | EXT4_SYNC_FL | \
	 ...
	 EXT4_NOATIME_FL | EXT4_JOURNAL_DATA_FL | EXT4_NOTAIL_FL |     \
	 EXT4_DIRSYNC_FL | EXT4_TOPDIR_FL | EXT4_EXTENTS_FL |          \
...
#define EXT4_FL_INHERITED                                              \
	(EXT4_SECRM_FL | EXT4_UNRM_FL | EXT4_COMPR_FL | EXT4_SYNC_FL | \
	 ...
	 EXT4_PROJINHERIT_FL | EXT4_CASEFOLD_FL | EXT4_DAX_FL)

/* Flags that are appropriate for regular files (all but dir-specific ones). */
#define EXT4_REG_FLMASK                                          \
	(~(EXT4_DIRSYNC_FL | EXT4_TOPDIR_FL | EXT4_CASEFOLD_FL | \
	   EXT4_PROJINHERIT_FL))
...
#define EXT4_INODE_SET_XTIME_VAL(xtime, inode, raw_inode, ts)             \
	do {                                                              \
		if (EXT4_FITS_IN_INODE(raw_inode, EXT4_I(inode),          \
				       xtime##_extra)) {                  \
			(raw_inode)->xtime = cpu_to_le32((ts).tv_sec);    \
			(raw_inode)->xtime##_extra =                      \
				ext4_encode_extra_time(ts);               \
		} else                                                    \
			(raw_inode)->xtime = cpu_to_le32(clamp_t(         \
				int32_t, (ts).tv_sec, S32_MIN, S32_MAX)); \
	} while (0)

#define EXT4_INODE_SET_ATIME(inode, raw_inode)              \
	EXT4_INODE_SET_XTIME_VAL(i_atime, inode, raw_inode, \
				 inode_get_atime(inode))

#define EXT4_INODE_SET_MTIME(inode, raw_inode)              \
	EXT4_INODE_SET_XTIME_VAL(i_mtime, inode, raw_inode, \
...
#define EXT4_INODE_GET_ATIME(inode, raw_inode)                                 \
	do {                                                                   \
		inode_set_atime_to_ts(inode,                                   \
				      EXT4_INODE_GET_XTIME_VAL(i_atime, inode, \
							       raw_inode));    \
	} while (0)
...
#define EXT4_INODE_GET_MTIME(inode, raw_inode)                                 \
	do {                                                                   \
		inode_set_mtime_to_ts(inode,                                   \
				      EXT4_INODE_GET_XTIME_VAL(i_mtime, inode, \
							       raw_inode));    \
	} while (0)
...
#define EXT4_INODE_GET_CTIME(inode, raw_inode)                                 \
	do {                                                                   \
		inode_set_ctime_to_ts(inode,                                   \
				      EXT4_INODE_GET_XTIME_VAL(i_ctime, inode, \
							       raw_inode));    \
	} while (0)
...
#define EXT4_EINODE_GET_XTIME(xtime, einode, raw_inode)                  \
	do {                                                             \
		if (EXT4_FITS_IN_INODE(raw_inode, einode, xtime))        \
			(einode)->xtime = EXT4_INODE_GET_XTIME_VAL(      \
				xtime, &(einode->vfs_inode), raw_inode); \
		else                                                     \
...
#define EXT4_S_ERR_START offsetof(struct ext4_super_block, s_error_count)
	...
	__u8 s_first_error_func
		[32] __nonstring; /* function where the error happened */
	__le32 s_first_error_line; /* line number where error happened */
	__le32 s_last_error_time; /* most recent time of an error */
	__le32 s_last_error_ino; /* inode involved in last error */
	__le32 s_last_error_line; /* line number where error happened */
	__le64 s_last_error_block; /* block involved of last error */
	__u8 s_last_error_func
		[32] __nonstring; /* function where the error happened */
...
static inline struct ext4_journal_trigger *
EXT4_TRIGGER(struct jbd2_buffer_trigger_type *trigger)
{
...
struct ext4_sb_info {
	...
	ext4_group_t
		s_blockfile_groups; /* Groups acceptable for non-extent files */
	...
	struct mutex
		s_error_notify_mutex; /* protects sysfs_notify vs kobject_del */
	...
	atomic64_t
		s_bal_cX_failed[EXT4_MB_NUM_CRS]; /* cX loop didn't find blocks */
	...
	struct ext4_journal_trigger
		s_journal_triggers[EXT4_JOURNAL_TRIGGER_COUNT];
...
{
	return le16_to_cpu(es->s_def_resuid) | le16_to_cpu(es->s_def_resuid_hi)
						       << 16;
}

static inline int ext4_get_resgid(struct ext4_super_block *es)
{
	return le16_to_cpu(es->s_def_resgid) | le16_to_cpu(es->s_def_resgid_hi)
						       << 16;
}
...
#define EXT4_INODE_BIT_FNS(name, field, offset)                                \
	...
	static inline void ext4_clear_inode_##name(struct inode *inode,        \
						   int bit)                    \
	{                                                                      \
...
static inline struct mapping_metadata_bhs *
ext4_i_metadata_bhs(struct inode *inode)
{
	/*
...
#define EXTN_FEATURE_FUNCS(ver)                                               \
	...
	{                                                                     \
		return ((EXT4_SB(sb)->s_es->s_feature_ro_compat &             \
			 cpu_to_le32(~EXT##ver##_FEATURE_RO_COMPAT_SUPP)) !=  \
			0);                                                   \
	}                                                                     \
	...
	{                                                                     \
		return ((EXT4_SB(sb)->s_es->s_feature_incompat &              \
			 cpu_to_le32(~EXT##ver##_FEATURE_INCOMPAT_SUPP)) !=   \
			0);                                                   \
	}
...
extern int __ext4_check_dir_entry(const char *, unsigned int, struct inode *,
				  struct file *, struct ext4_dir_entry_2 *,
...
#define ext4_new_inode_start_handle(idmap, dir, mode, qstr, goal, owner, type, \
				    nblocks)                                   \
	__ext4_new_inode((idmap), NULL, (dir), (mode), (qstr), (goal),         \
			 (owner), 0, (type), __LINE__, (nblocks))
...
int ext4_fc_record_regions(struct super_block *sb, int ino, ext4_lblk_t lblk,
			   ext4_fsblk_t pblk, int len, int replay);
...
extern ext4_group_t ext4_mb_prefetch(struct super_block *sb, ext4_group_t group,
...
extern int ext4_mb_add_groupinfo(struct super_block *sb, ext4_group_t i,
				 struct ext4_group_desc *desc);
extern int ext4_group_add_blocks(handle_t *handle, struct super_block *sb,
				 ext4_fsblk_t block, unsigned long count);
extern int ext4_trim_fs(struct super_block *, struct fstrim_range *);
extern void ext4_process_freed_data(struct super_block *sb, tid_t commit_tid);
extern void ext4_mb_mark_bb(struct super_block *sb, ext4_fsblk_t block, int len,
			    bool state);
...
int ext4_walk_page_buffers(handle_t *handle, struct inode *inode,
			   struct buffer_head *head, unsigned from, unsigned to,
			   int *partial,
...
extern int ext4_getattr(struct mnt_idmap *, const struct path *, struct kstat *,
			u32, unsigned int);
...
int ext4_fileattr_set(struct mnt_idmap *idmap, struct dentry *dentry,
...
extern int ext4_search_dir(struct buffer_head *bh, char *search_buf,
			   int buf_size, struct inode *dir,
			   struct ext4_filename *fname, unsigned int offset,
...
extern __printf(7, 8) void __ext4_error(struct super_block *, const char *,
					unsigned int, bool, int, __u64,
					const char *, ...);
extern __printf(6, 7) void __ext4_error_inode(struct inode *, const char *,
					      unsigned int, ext4_fsblk_t, int,
					      const char *, ...);
extern __printf(5, 6) void __ext4_error_file(struct file *, const char *,
					     unsigned int, ext4_fsblk_t,
					     const char *, ...);
extern void __ext4_std_error(struct super_block *, const char *, unsigned int,
			     int);
extern __printf(4, 5) void __ext4_warning(struct super_block *, const char *,
					  unsigned int, const char *, ...);
extern __printf(4, 5) void __ext4_warning_inode(const struct inode *inode,
						const char *function,
						unsigned int line,
						const char *fmt, ...);
extern __printf(3, 4) void __ext4_msg(struct super_block *, const char *,
				      const char *, ...);
...
#define ext4_error_inode_block(inode, block, err, fmt, a...)                   \
	__ext4_error_inode((inode), __func__, __LINE__, (block), (err), (fmt), \
			   ##a)
...
extern int ext4_try_create_inline_dir(handle_t *handle, struct inode *parent,
				      struct inode *inode);
extern int ext4_read_inline_dir(struct file *filp, struct dir_context *ctx,
				int *has_inline_data);
extern int ext4_inlinedir_to_tree(struct file *dir_file, struct inode *dir,
				  ext4_lblk_t block, struct dx_hash_info *hinfo,
				  __u32 start_hash, __u32 start_minor_hash,
				  int *has_inline_data);
extern struct buffer_head *
ext4_find_inline_entry(struct inode *dir, struct ext4_filename *fname,
		       struct ext4_dir_entry_2 **res_dir, int *has_inline_data);
extern int ext4_delete_inline_entry(handle_t *handle, struct inode *dir,
				    struct ext4_dir_entry_2 *de_del,
				    struct buffer_head *bh,
				    int *has_inline_data);
...
ext4_get_first_inline_block(struct inode *inode,
			    struct ext4_dir_entry_2 **parent_de, int *retval);
...
static inline void ext4_set_de_type(struct super_block *sb,
				    struct ext4_dir_entry_2 *de, umode_t mode)
{
...
extern int ext4_inode_block_valid(struct inode *inode, ext4_fsblk_t start_blk,
				  unsigned int count);
extern int ext4_check_blockref(const char *, unsigned int, struct inode *,
			       __le32 *, unsigned int);
...
extern int ext4_move_extents(struct file *o_filp, struct file *d_filp,
			     __u64 start_orig, __u64 start_donor, __u64 len,
			     __u64 *moved_len);
...
extern void
ext4_orphan_file_block_trigger(struct jbd2_buffer_trigger_type *triggers,
			       struct buffer_head *bh, void *data, size_t size);
