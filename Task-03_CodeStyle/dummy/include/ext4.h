...
#define ext4_debug(f, a...)                                              \
  do {                                                                   \
    printk(KERN_DEBUG "EXT4-fs DEBUG (%s, %d): %s:", __FILE__, __LINE__, \
           __func__);                                                    \
...
#define ext_debug(ino, fmt, ...)                                               \
  pr_debug("[%s/%d] EXT4-fs (%s): ino %llu: (%s, %d): %s:" fmt, current->comm, \
           task_pid_nr(current), ino->i_sb->s_id, ino->i_ino, __FILE__,        \
...
#define EXT4_MAP_FLAGS                                                       \
  (EXT4_MAP_NEW | EXT4_MAP_MAPPED | EXT4_MAP_UNWRITTEN | EXT4_MAP_BOUNDARY | \
...
#define EXT4_IO_END_DEFER_COMPLETION \
  (EXT4_IO_END_UNWRITTEN | EXT4_IO_END_FAILED)
...
#define EXT4_INODE_SIZE(s)                                            \
  (((s)->s_rev_level == EXT4_GOOD_OLD_REV) ? EXT4_GOOD_OLD_INODE_SIZE \
                                           : (s)->s_inode_size)
#define EXT4_FIRST_INO(s)                                            \
  (((s)->s_rev_level == EXT4_GOOD_OLD_REV) ? EXT4_GOOD_OLD_FIRST_INO \
                                           : (s)->s_first_ino)
#endif
#define EXT4_BLOCK_ALIGN(size, blkbits) ALIGN((size), (1 << (blkbits)))
#define EXT4_MAX_BLOCKS(size, offset, blkbits) \
  ((EXT4_BLOCK_ALIGN(size + offset, blkbits) >> blkbits) - (offset >> blkbits))
...
#define EXT4_PBLK_CMASK(s, pblk) \
  ((pblk) & ~((ext4_fsblk_t)(s)->s_cluster_ratio - 1))
#define EXT4_LBLK_CMASK(s, lblk) \
  ((lblk) & ~((ext4_lblk_t)(s)->s_cluster_ratio - 1))
...
#define EXT4_BG_INODE_BITMAP_CSUM_HI_END \
  (offsetof(struct ext4_group_desc, bg_inode_bitmap_csum_hi) + sizeof(__le16))
#define EXT4_BG_BLOCK_BITMAP_CSUM_HI_END \
  (offsetof(struct ext4_group_desc, bg_block_bitmap_csum_hi) + sizeof(__le16))
...
#define EXT4_FL_USER_MODIFIABLE                                               \
  (EXT4_SECRM_FL | EXT4_UNRM_FL | EXT4_COMPR_FL | EXT4_SYNC_FL |              \
   EXT4_IMMUTABLE_FL | EXT4_APPEND_FL | EXT4_NODUMP_FL | EXT4_NOATIME_FL |    \
   EXT4_JOURNAL_DATA_FL | EXT4_NOTAIL_FL | EXT4_DIRSYNC_FL | EXT4_TOPDIR_FL | \
   EXT4_EXTENTS_FL | 0x00400000 /* EXT4_EOFBLOCKS_FL */ | EXT4_DAX_FL |       \
   EXT4_PROJINHERIT_FL | EXT4_CASEFOLD_FL)

/* User visible flags */
#define EXT4_FL_USER_VISIBLE                                            \
  (EXT4_FL_USER_MODIFIABLE | EXT4_DIRTY_FL | EXT4_COMPRBLK_FL |         \
   EXT4_NOCOMPR_FL | EXT4_ENCRYPT_FL | EXT4_INDEX_FL | EXT4_VERITY_FL | \
   EXT4_INLINE_DATA_FL)

/* Flags that should be inherited by new inodes from their parent. */
#define EXT4_FL_INHERITED                                                      \
  (EXT4_SECRM_FL | EXT4_UNRM_FL | EXT4_COMPR_FL | EXT4_SYNC_FL |               \
   EXT4_NODUMP_FL | EXT4_NOATIME_FL | EXT4_NOCOMPR_FL | EXT4_JOURNAL_DATA_FL | \
   EXT4_NOTAIL_FL | EXT4_DIRSYNC_FL | EXT4_PROJINHERIT_FL | EXT4_CASEFOLD_FL | \
   EXT4_DAX_FL)

/* Flags that are appropriate for regular files (all but dir-specific ones). */
#define EXT4_REG_FLMASK \
  (~(EXT4_DIRSYNC_FL | EXT4_TOPDIR_FL | EXT4_CASEFOLD_FL | EXT4_PROJINHERIT_FL))
...
#define EXT4_DAX_MUT_EXCL                                    \
  (EXT4_VERITY_FL | EXT4_ENCRYPT_FL | EXT4_JOURNAL_DATA_FL | \
   EXT4_INLINE_DATA_FL)
...
#define EXT4_GET_BLOCKS_CREATE_UNWRIT_EXT \
  (EXT4_GET_BLOCKS_UNWRIT_EXT | EXT4_GET_BLOCKS_CREATE)
...
#define EXT4_GET_BLOCKS_CREATE_ZERO \
  (EXT4_GET_BLOCKS_CREATE | EXT4_GET_BLOCKS_ZERO)
...
#define EXT4_GET_BLOCKS_IO_CONVERT_EXT                           \
  (EXT4_GET_BLOCKS_CONVERT | EXT4_GET_BLOCKS_CREATE_UNWRIT_EXT | \
 ...
 */
#define EXT4_EX_QUERY_FILTER                                \
  (EXT4_EX_NOCACHE | EXT4_EX_FORCE_CACHE | EXT4_EX_NOFAIL | \
...
#define EXT4_INODE_GET_XTIME_VAL(xtime, inode, raw_inode)      \
  (EXT4_FITS_IN_INODE(raw_inode, EXT4_I(inode), xtime##_extra) \
       ...
       : (struct timespec64){.tv_sec =                         \
                                 (signed)le32_to_cpu((raw_inode)->xtime)})

#define EXT4_INODE_GET_ATIME(inode, raw_inode)                       \
  do {                                                               \
    inode_set_atime_to_ts(                                           \
        inode, EXT4_INODE_GET_XTIME_VAL(i_atime, inode, raw_inode)); \
  } while (0)

#define EXT4_INODE_GET_MTIME(inode, raw_inode)                       \
  do {                                                               \
    inode_set_mtime_to_ts(                                           \
        inode, EXT4_INODE_GET_XTIME_VAL(i_mtime, inode, raw_inode)); \
  } while (0)

#define EXT4_INODE_GET_CTIME(inode, raw_inode)                       \
  do {                                                               \
    inode_set_ctime_to_ts(                                           \
        inode, EXT4_INODE_GET_XTIME_VAL(i_ctime, inode, raw_inode)); \
  } while (0)
...
#define EXT4_EINODE_GET_XTIME(xtime, einode, raw_inode)                     \
  do {                                                                      \
    if (EXT4_FITS_IN_INODE(raw_inode, einode, xtime))                       \
      (einode)->xtime =                                                     \
          EXT4_INODE_GET_XTIME_VAL(xtime, &(einode->vfs_inode), raw_inode); \
    else                                                                    \
...
#define EXT4_MOUNT_USRQUOTA                \
  0x80000 /* "old" user quota,             \
           * enable enforcement for hidden \
           * quota files */
#define EXT4_MOUNT_GRPQUOTA                \
  0x100000 /* "old" group quota, enable    \
            * enforcement for hidden quota \
            * files */
#define EXT4_MOUNT_PRJQUOTA        \
  0x200000 /* Enable project quota \
            * enforcement */
#define EXT4_MOUNT_DIOREAD_NOLOCK \
  0x400000 /* Enable support for dio read nolocking */
...
#define EXT4_MOUNT2_STD_GROUP_SIZE                                    \
  0x00000002 /* We have standard group                                \
                                                size of blocksize * 8 \
                                                blocks */
#define EXT4_MOUNT2_HURD_COMPAT        \
  0x00000004 /* Support HURD-castrated \
                                                file systems */
#define EXT4_MOUNT2_EXPLICIT_JOURNAL_CHECKSUM \
  0x00000008 /* User explicitly               \
...
#define EXT4_MOUNT2_MB_OPTIMIZE_SCAN                        \
  0x00000080                         /* Optimize group      \
                                      * scanning in mballoc \
                                      */
#define EXT4_MOUNT2_ABORT 0x00000100 /* Abort filesystem */

#define clear_opt(sb, opt) EXT4_SB(sb)->s_mount_opt &= ~EXT4_MOUNT_##opt
...
#define clear_opt2(sb, opt) EXT4_SB(sb)->s_mount_opt2 &= ~EXT4_MOUNT2_##opt
#define set_opt2(sb, opt) EXT4_SB(sb)->s_mount_opt2 |= EXT4_MOUNT2_##opt
#define test_opt2(sb, opt) (EXT4_SB(sb)->s_mount_opt2 & EXT4_MOUNT2_##opt)
...
#define EXT4_S_ERR_START offsetof(struct ext4_super_block, s_error_count)
  ...
  __u8
      s_last_error_func[32] __nonstring; /* function where the error happened */
...
static inline int ext4_get_resuid(struct ext4_super_block* es) {
  return le16_to_cpu(es->s_def_resuid) | le16_to_cpu(es->s_def_resuid_hi) << 16;
}

static inline int ext4_get_resgid(struct ext4_super_block* es) {
  return le16_to_cpu(es->s_def_resgid) | le16_to_cpu(es->s_def_resgid_hi) << 16;
}
...
#define EXT4_FEATURE_COMPAT_SUPP \
  (EXT4_FEATURE_COMPAT_EXT_ATTR | EXT4_FEATURE_COMPAT_ORPHAN_FILE)
#define EXT4_FEATURE_INCOMPAT_SUPP                                     \
  (EXT4_FEATURE_INCOMPAT_FILETYPE | EXT4_FEATURE_INCOMPAT_RECOVER |    \
   EXT4_FEATURE_INCOMPAT_META_BG | EXT4_FEATURE_INCOMPAT_EXTENTS |     \
   EXT4_FEATURE_INCOMPAT_64BIT | EXT4_FEATURE_INCOMPAT_FLEX_BG |       \
   EXT4_FEATURE_INCOMPAT_EA_INODE | EXT4_FEATURE_INCOMPAT_MMP |        \
   EXT4_FEATURE_INCOMPAT_INLINE_DATA | EXT4_FEATURE_INCOMPAT_ENCRYPT | \
   EXT4_FEATURE_INCOMPAT_CASEFOLD | EXT4_FEATURE_INCOMPAT_CSUM_SEED |  \
...
#define EXT4_FEATURE_RO_COMPAT_SUPP                                          \
  (EXT4_FEATURE_RO_COMPAT_SPARSE_SUPER | EXT4_FEATURE_RO_COMPAT_LARGE_FILE | \
   ...
   EXT4_FEATURE_RO_COMPAT_HUGE_FILE | EXT4_FEATURE_RO_COMPAT_BIGALLOC |      \
   EXT4_FEATURE_RO_COMPAT_METADATA_CSUM | EXT4_FEATURE_RO_COMPAT_QUOTA |     \
...
#define EXT4_DIRENT_HASHES(entry)                                            \
  ((struct ext4_dir_entry_hash*)(((void*)(entry)) +                          \
                                 ((8 + (entry)->name_len + EXT4_DIR_ROUND) & \
                                  ~EXT4_DIR_ROUND)))
...
#define EXT4_DIR_LINK_MAX(dir)                \
  unlikely((dir)->i_nlink >= EXT4_LINK_MAX && \
...
static inline u32 ext4_chksum(u32 crc,
                              const void* address,
                              unsigned int length) {
...
static inline bool ext4_is_quota_file(struct inode* inode) {
  return IS_NOQUOTA(inode) && !(EXT4_I(inode)->i_flags & EXT4_EA_INODE_FL);
}
...
static inline int ext4_dir_htree_level(struct super_block* sb) {
  return ext4_has_feature_largedir(sb) ? EXT4_HTREE_LEVEL
...
extern unsigned long ext4_bg_num_gdb(struct super_block* sb,
                                     ext4_group_t group);
extern ext4_fsblk_t ext4_new_meta_blocks(handle_t* handle,
                                         struct inode* inode,
                                         ext4_fsblk_t goal,
                                         unsigned int flags,
                                         unsigned long* count,
...
extern struct buffer_head* ext4_read_block_bitmap_nowait(
    struct super_block* sb,
    ext4_group_t block_group,
    bool ignore_locked);
...
int ext4_fname_prepare_lookup(struct inode* dir,
...
/* dir.c */
extern int __ext4_check_dir_entry(const char*,
                                  unsigned int,
                                  struct inode*,
                                  struct file*,
                                  struct ext4_dir_entry_2*,
...
#define ext4_check_dir_entry(dir, filp, de, bh, buf, size, offset)         \
  unlikely(__ext4_check_dir_entry(__func__, __LINE__, (dir), (filp), (de), \
                                  (bh), (buf), (size), (offset)))
extern int ext4_htree_store_dirent(struct file* dir_file,
                                   __u32 hash,
                                   __u32 minor_hash,
                                   struct ext4_dir_entry_2* dirent,
                                   struct fscrypt_str* ent_name);
extern void ext4_htree_free_dir_info(struct dir_private_info* p);
extern int ext4_find_dest_de(struct inode* dir,
                             struct buffer_head* bh,
                             void* buf,
                             int buf_size,
                             struct ext4_filename* fname,
...
extern int ext4_check_all_de(struct inode* dir,
                             struct buffer_head* bh,
                             void* buf,
                             int buf_size);
...
/* hash.c */
extern int ext4fs_dirhash(const struct inode* dir,
                          const char* name,
                          int len,
                          struct dx_hash_info* hinfo);
...
extern struct inode* __ext4_new_inode(struct mnt_idmap*,
                                      handle_t*,
                                      struct inode*,
                                      umode_t,
...
#define ext4_new_inode(handle, dir, mode, qstr, goal, owner, i_flags)       \
  __ext4_new_inode(&nop_mnt_idmap, (handle), (dir), (mode), (qstr), (goal), \
                   (owner), i_flags, 0, 0, 0)
#define ext4_new_inode_start_handle(idmap, dir, mode, qstr, goal, owner, type, \
                                    nblocks)                                   \
  __ext4_new_inode((idmap), NULL, (dir), (mode), (qstr), (goal), (owner), 0,   \
...
void ext4_fc_track_range(handle_t* handle,
                         struct inode* inode,
                         ext4_lblk_t start,
                         ext4_lblk_t end);
...
void ext4_fc_track_link(handle_t* handle,
                        struct inode* inode,
                        struct dentry* dentry);
void __ext4_fc_track_create(handle_t* handle,
                            struct inode* inode,
                            struct dentry* dentry);
void ext4_fc_track_create(handle_t* handle, struct dentry* dentry);
void ext4_fc_track_inode(handle_t* handle, struct inode* inode);
void ext4_fc_mark_ineligible(struct super_block* sb,
                             int reason,
                             handle_t* handle);
...
int ext4_fc_record_regions(struct super_block* sb,
                           int ino,
                           ext4_lblk_t lblk,
                           ext4_fsblk_t pblk,
                           int len,
...
extern void ext4_free_blocks(handle_t* handle,
                             struct inode* inode,
                             struct buffer_head* bh,
                             ext4_fsblk_t block,
                             unsigned long count,
...
extern int ext4_group_add_blocks(handle_t* handle,
                                 struct super_block* sb,
                                 ext4_fsblk_t block,
                                 unsigned long count);
extern int ext4_trim_fs(struct super_block*, struct fstrim_range*);
extern void ext4_process_freed_data(struct super_block* sb, tid_t commit_tid);
extern void ext4_mb_mark_bb(struct super_block* sb,
                            ext4_fsblk_t block,
                            int len,
                            bool state);
...
/* inode.c */
void ext4_inode_csum_set(struct inode* inode,
...
int ext4_bread_batch(struct inode* inode,
                     ext4_lblk_t block,
                     int bh_count,
                     bool wait,
                     struct buffer_head** bhs);
int ext4_get_block_unwritten(struct inode* inode,
                             sector_t iblock,
...
int ext4_get_block(struct inode* inode,
                   ...
                   struct buffer_head* bh_result,
                   int create);
int ext4_da_get_block_prep(struct inode* inode,
                           sector_t iblock,
                           struct buffer_head* bh,
                           int create);
...
extern struct inode* __ext4_iget(struct super_block* sb,
                                 unsigned long ino,
...
extern int ext4_setattr(struct mnt_idmap*, struct dentry*, struct iattr*);
extern u32 ext4_dio_alignment(struct inode* inode);
extern int ext4_getattr(struct mnt_idmap*,
                        const struct path*,
                        struct kstat*,
                        u32,
                        unsigned int);
...
extern int ext4_file_getattr(struct mnt_idmap*,
                             const struct path*,
                             struct kstat*,
                             u32,
                             unsigned int);
...
int ext4_get_fc_inode_loc(struct super_block* sb,
                          unsigned long ino,
...
extern int ext4_meta_trans_blocks(struct inode* inode,
                                  int lblocks,
                                  int pextents,
                                  int alloc_extents);
...
extern void ext4_da_update_reserve_space(struct inode* inode,
                                         int used,
                                         int quota_claim);
extern int ext4_issue_zeroout(struct inode* inode,
                              ext4_lblk_t lblk,
                              ext4_fsblk_t pblk,
                              ext4_lblk_t len);
...
/* indirect.c */
extern int ext4_ind_map_blocks(handle_t* handle,
                               struct inode* inode,
                               struct ext4_map_blocks* map,
                               int flags);
extern int ext4_ind_trans_blocks(struct inode* inode, int nrblocks);
extern void ext4_ind_truncate(handle_t*, struct inode* inode);
extern int ext4_ind_remove_space(handle_t* handle,
                                 struct inode* inode,
                                 ext4_lblk_t start,
                                 ext4_lblk_t end);
...
/* namei.c */
extern int ext4_init_new_dir(handle_t* handle,
                             struct inode* dir,
                             struct inode* inode);
extern int ext4_dirblock_csum_verify(struct inode* inode,
                                     struct buffer_head* bh);
extern int ext4_htree_fill_tree(struct file* dir_file,
                                __u32 start_hash,
...
int ext4_read_bh(struct buffer_head* bh,
                 ...
                 bio_end_io_t end_io,
                 bool simu_fail);
extern int ext4_read_bh_lock(struct buffer_head* bh,
                             blk_opf_t op_flags,
                             bool wait);
...
extern int ext4_alloc_flex_bg_array(struct super_block* sb,
                                    ext4_group_t ngroup);
extern const char* ext4_decode_error(struct super_block* sb,
                                     int errno,
                                     char nbuf[16]);
...
extern __printf(7, 8) void __ext4_error(struct super_block*,
                                        const char*,
                                        unsigned int,
                                        bool,
                                        int,
                                        ...
                                        const char*,
                                        ...);
extern __printf(6, 7) void __ext4_error_inode(struct inode*,
                                              const char*,
                                              unsigned int,
                                              ext4_fsblk_t,
                                              int,
                                              const char*,
                                              ...);
extern __printf(5, 6) void __ext4_error_file(struct file*,
                                             const char*,
                                             unsigned int,
                                             ext4_fsblk_t,
                                             const char*,
                                             ...);
extern void __ext4_std_error(struct super_block*,
                             const char*,
                             unsigned int,
                             int);
extern __printf(4, 5) void __ext4_warning(struct super_block*,
                                          const char*,
                                          unsigned int,
                                          const char*,
                                          ...);
extern __printf(4, 5) void __ext4_warning_inode(const struct inode* inode,
                                                const char* function,
                                                unsigned int line,
                                                const char* fmt,
                                                ...);
extern __printf(3, 4) void __ext4_msg(struct super_block*,
                                      const char*,
                                      const char*,
                                      ...);
extern void __dump_mmp_msg(struct super_block*,
                           struct mmp_struct* mmp,
                           const char*,
                           unsigned int,
                           const char*);
extern __printf(7, 8) void __ext4_grp_locked_error(const char*,
                                                   unsigned int,
                                                   struct super_block*,
                                                   ext4_group_t,
                                                   u64,
...
#define ext4_error_inode_block(inode, block, err, fmt, a...) \
  __ext4_error_inode((inode), __func__, __LINE__, (block), (err), (fmt), ##a)
...
#define ext4_error(sb, fmt, ...) \
  __ext4_error((sb), __func__, __LINE__, false, 0, 0, (fmt), ##__VA_ARGS__)
#define ext4_error_err(sb, err, fmt, ...) \
  __ext4_error((sb), __func__, __LINE__, false, (err), 0, (fmt), ##__VA_ARGS__)
...
#define ext4_grp_locked_error(sb, grp, ino, block, fmt, ...)            \
  __ext4_grp_locked_error(__func__, __LINE__, sb, grp, ino, block, fmt, \
                          ##__VA_ARGS__)
...
extern void ext4_itable_unused_set(struct super_block* sb,
                                   struct ext4_group_desc* bg,
                                   __u32 count);
extern int ext4_group_desc_csum_verify(struct super_block* sb,
                                       __u32 group,
                                       struct ext4_group_desc* gdp);
extern void ext4_group_desc_csum_set(struct super_block* sb,
                                     __u32 group,
                                     struct ext4_group_desc* gdp);
...
static inline int ext4_has_group_desc_csum(struct super_block* sb) {
  return ext4_has_feature_gdt_csum(sb) || ext4_has_feature_metadata_csum(sb);
}
...
static inline loff_t ext4_isize(struct super_block* sb,
  ...
  if (ext4_has_feature_largedir(sb) || S_ISREG(le16_to_cpu(raw_inode->i_mode)))
...
int ext4_update_disksize_before_punch(struct inode* inode,
                                      loff_t offset,
                                      loff_t len);
...
static inline bool ext4_try_lock_group(struct super_block* sb,
                                       ext4_group_t group) {
...
static inline void ext4_lock_group(struct super_block* sb, ext4_group_t group) {
  if (!ext4_try_lock_group(sb, group)) {
     ...
     */
    atomic_add_unless(&EXT4_SB(sb)->s_lock_busy, 1, EXT4_MAX_CONTENTION);
...
static inline bool ext4_is_quota_journalled(struct super_block* sb) {
  ...
  return (ext4_has_feature_quota(sb) || sbi->s_qf_names[USRQUOTA] ||
...
#define ext4_check_indirect_blockref(inode, bh)                         \
  ext4_check_blockref(__func__, __LINE__, inode, (__le32*)(bh)->b_data, \
                      EXT4_ADDR_PER_BLOCK((inode)->i_sb))

#define ext4_ind_check_inode(inode)                                     \
  ext4_check_blockref(__func__, __LINE__, inode, EXT4_I(inode)->i_data, \
                      EXT4_NDIR_BLOCKS)
...
extern int ext4_inlinedir_to_tree(struct file* dir_file,
                                  ...
                                  __u32 start_minor_hash,
                                  int* has_inline_data);
extern struct buffer_head* ext4_find_inline_entry(
    struct inode* dir,
    struct ext4_filename* fname,
    struct ext4_dir_entry_2** res_dir,
    int* has_inline_data);
...
extern int ext4_init_dirblock(handle_t* handle,
                              struct inode* inode,
...
extern int ext4_handle_dirty_dirblock(handle_t* handle,
                                      struct inode* inode,
                                      struct buffer_head* bh);
extern int __ext4_unlink(struct inode* dir,
                         const struct qstr* d_name,
                         struct inode* inode,
                         struct dentry* dentry);
extern int __ext4_link(struct inode* dir,
                       struct inode* inode,
                       const struct qstr* d_name,
                       struct dentry* dentry);
...
extern int ext4_inode_block_valid(struct inode* inode,
                                  ...
                                  unsigned int count);
extern int ext4_check_blockref(const char*,
                               unsigned int,
                               struct inode*,
                               __le32*,
                               unsigned int);
extern int ext4_sb_block_valid(struct super_block* sb,
                               struct inode* inode,
                               ext4_fsblk_t start_blk,
                               unsigned int count);
...
extern int ext4_ext_map_blocks(handle_t* handle,
                               struct inode* inode,
                               struct ext4_map_blocks* map,
                               int flags);
extern int ext4_ext_truncate(handle_t*, struct inode*);
extern int ext4_ext_remove_space(struct inode* inode,
                                 ext4_lblk_t start,
                                 ext4_lblk_t end);
extern void ext4_ext_init(struct super_block*);
extern void ext4_ext_release(struct super_block*);
extern long ext4_fallocate(struct file* file,
                           int mode,
                           loff_t offset,
                           loff_t len);
extern int ext4_convert_unwritten_extents(handle_t* handle,
                                          struct inode* inode,
                                          loff_t offset,
                                          ssize_t len,
...
extern int ext4_convert_unwritten_io_end_vec(handle_t* handle,
                                             ext4_io_end_t* io_end);
extern int ext4_map_blocks(handle_t* handle,
                           struct inode* inode,
                           struct ext4_map_blocks* map,
                           int flags);
extern int ext4_map_query_blocks(handle_t* handle,
                                 struct inode* inode,
                                 struct ext4_map_blocks* map,
                                 int flags);
extern int ext4_map_create_blocks(handle_t* handle,
                                  struct inode* inode,
                                  struct ext4_map_blocks* map,
                                  int flags);
...
extern struct ext4_ext_path* ext4_ext_insert_extent(handle_t* handle,
                                                    struct inode* inode,
                                                    struct ext4_ext_path* path,
                                                    struct ext4_extent* newext,
                                                    int gb_flags);
extern struct ext4_ext_path* ext4_find_extent(struct inode*,
                                              ext4_lblk_t,
                                              struct ext4_ext_path*,
                                              int flags);
...
extern int ext4_fiemap(struct inode* inode,
...
extern int ext4_swap_extents(handle_t* handle,
                             struct inode* inode1,
                             struct inode* inode2,
                             ext4_lblk_t lblk1,
                             ext4_lblk_t lblk2,
...
extern int ext4_datasem_ensure_credits(handle_t* handle,
                                       struct inode* inode,
                                       int check_cred,
                                       int restart_cred,
                                       int revoke_cred);
...
extern int ext4_ext_replay_update_ex(struct inode* inode,
                                     ext4_lblk_t start,
                                     int len,
                                     int unwritten,
                                     ext4_fsblk_t pblk);
...
extern int ext4_move_extents(struct file* o_filp,
                             struct file* d_filp,
                             __u64 start_orig,
                             __u64 start_donor,
                             __u64 len,
...
void ext4_bio_write_folio(struct ext4_io_submit* io,
                          struct folio* page,
                          size_t len);
...
static inline int bitmap_uptodate(struct buffer_head* bh) {
  return (buffer_uptodate(bh) && test_bit(BH_BITMAP_UPTODATE, &(bh)->b_state));
}
...
int ext4_iomap_begin(struct inode* inode,
                     loff_t offset,
                     loff_t length,
                     unsigned flags,
                     struct iomap* iomap,
...
extern int ext4_block_write_begin(handle_t* handle,
                                  struct folio* folio,
                                  loff_t pos,
                                  unsigned len,
